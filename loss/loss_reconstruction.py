...
def loss_reconstruction(prediction, target, sum_loss):
    ...
    print(t_pts.unsqueeze(1).unsqueeze(3).expand(-1,N_p,-1,M,-1).shape, p_pts.unsqueeze(2).expand(-1,-1,N_t,-1,-1).shape)
    dist =  t_pts.unsqueeze(1).unsqueeze(3).expand(-1,N_p,-1,M,-1) - p_pts.unsqueeze(2).expand(-1,-1,N_t,-1,-1)
    # batch * N_p * N_t * M * 3
    dist = dist.pow(2).sum(-1) # batch * M * N_p * N_t
    min_dist = dist.min(3)[0].min(1)[0] + dist.min(2)[0] # batch
    loss = min_dist.mean()
